...
<table<?php print $atomium['attributes']['wrapper']; ?>>
  ...
  <?php if (!empty($header)) : ?>
    ...
    <?php foreach ($header as $row_index => $row): ?>
      <tr<?php print $atomium['attributes']['header-row-' . $row_num_index]->append('class', $row_index); ?>>
        <?php $cell_num_index = 0; ?>
        <?php foreach ($row['cells'] as $cell_index => $cell): ?>
  ...
  <?php foreach ($rows as $row_index => $row): ?>
    <tr<?php print $atomium['attributes']['rows-row-' . $row_num_index]->append('class', $row_index); ?>>
      <?php $cell_num_index = 0; ?>
      <?php foreach ($row['cells'] as $cell_index => $cell): ?>
  ...
  <?php if (!empty($footer)) : ?>
    ...
    <?php foreach ($footer as $row_index => $row): ?>
      <tr<?php print $atomium['attributes']['footer-row-' . $row_num_index]->append('class', $row_index); ?>>
        <?php $cell_num_index = 0; ?>
        <?php foreach ($row['cells'] as $cell_index => $cell): ?>
